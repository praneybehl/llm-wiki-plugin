...
4b. Run `python skills/llm-wiki/scripts/wiki_search.py "<query terms>" --top 10 --cache --json` after resolving the mode. Add `--no-embed` for `lexical`, `deferred`, or unanswered setup. Use `--type concept` / `--tag <tag>` when helpful and `--granularity page` for whole-page ranking.
