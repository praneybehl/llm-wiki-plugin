...
4. After all sections are merged or skipped, mention that:
   ...
   **v3.0.0 notes** (mention these too):
   ...
   - `python "${CLAUDE_PLUGIN_ROOT}/skills/llm-wiki/scripts/wiki_search.py" "<query>" --wiki "<absolute-project-root>/wiki" --no-embed` forces dependency-free lexical BM25. Missing packages, model initialization errors, and sqlite-vec load failures also fall back to lexical mode without breaking query output.
