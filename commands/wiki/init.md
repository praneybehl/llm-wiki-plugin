...
5. If I selected hybrid retrieval, explain before asking for approval that the first build sends all canonical section text, every hybrid query sends its query text, and new or changed sections are sent when their vectors are missing; these provider requests may be billable. Then ask for explicit approval before the first build. Tell me which environment variables to set through my shell or secret manager, but never collect or store their values.
