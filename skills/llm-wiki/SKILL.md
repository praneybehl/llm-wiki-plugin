...
For “learn from this task”, `/wiki:learn`, “improve this procedure” or `/wiki:evolve`, read `references/evolution-workflow.md`. Use `scripts/wiki_evolve.py` to capture immutable observed experience, snapshot evidence-linked single-file proposals, compare baseline/candidate skills through a trusted runner, retain rejected trials, and explicitly apply or roll back passing changes. This is an optional operator-driven workflow, not a background optimizer. Normal wiki use is unchanged.
