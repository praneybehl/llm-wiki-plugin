...
2b. If `wiki/graph/ontology.yaml` exists, also run `uv run --script skills/llm-wiki/scripts/wiki_graph_lint.py wiki/`. Triage typed-edge findings (unknown predicates, missing evidence, broken object refs, alias collisions) the same way as structural ones — propose fixes, don't apply silently. After approved fixes, suggest running `uv run --script skills/llm-wiki/scripts/wiki_graph_extract.py wiki/`.
