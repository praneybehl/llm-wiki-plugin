...
4b. Run `uv run --script skills/llm-wiki/scripts/wiki_search.py "<query terms>" --top 10 --cache --json`. When deterministic dependency-free lexical retrieval is desired, run `python skills/llm-wiki/scripts/wiki_search.py "<query terms>" --top 10 --cache --json --no-embed` instead; do not append `--no-embed` to the `uv run --script` form because uv still resolves the declared semantic packages. Use `--type concept` / `--tag <tag>` when helpful and `--granularity page` for whole-page ranking.
