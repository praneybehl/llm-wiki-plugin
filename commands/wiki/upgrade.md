...
4. After all sections are merged or skipped, mention that:
   ...
   **v2.0.0 notes** (mention these too):
   ...
   - Embeddings are opt-in: hybrid retrieval requires both `Embedding mode: openai | custom` in `SCHEMA.md` and provider configuration through `LLM_WIKI_EMBED_URL` (or `OPENAI_API_KEY`), optionally with `LLM_WIKI_EMBED_KEY` and `LLM_WIKI_EMBED_MODEL` (default `text-embedding-3-small`). `undecided`, `lexical`, and `deferred` remain local-only even when a key is present; `--no-embed` always forces lexical.
