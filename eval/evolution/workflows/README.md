...
1. Install and authenticate the selected native CLIs. Copy `../pilot/config.example.json`; use absolute runner paths and exact supported model IDs. Set two iterations, two repeats, `max_calls: 260`, `max_seconds: 7200`, `timeout: 480` and `max_usd: null`. Add a Claude transfer runner to reproduce the second-agent comparison.
