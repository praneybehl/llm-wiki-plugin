...
4. If the index doesn't surface good candidates, fall back to `python skills/llm-wiki/scripts/wiki_search.py "<query terms>" --top 10 --json` for BM25-ranked retrieval. Use `--type concept` / `--tag <tag>` to filter if helpful; use `--granularity page` for whole-page ranking. Hybrid retrieval (semantic + BM25) activates automatically when the embedding env vars are configured.
